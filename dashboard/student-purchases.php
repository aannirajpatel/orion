<?php
/**
 * Written by Aan (aancodes@gmail.com) at Global BizConnect on 25/5/19 2:31 PM.
 */
...
<head>
    ...
    <title><?php echo $_SESSION['fname'] . "'s" ?> Purchases</title>
                ...
                <!-- Content Row -->
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                            <?php
                            $getPurchaseQuery = "SELECT * FROM cpurchases WHERE uid=$uid";
                            $getPurchaseResult = mysqli_query($con, $getPurchaseQuery) or die(mysqli_error($con));
                            if (mysqli_num_rows($getPurchaseResult) == 0) {
                                echo "<tr><td colspan='4'>No purchases to show</td></tr>";
                            }
                            while ($getPurchaseData = mysqli_fetch_array($getPurchaseResult)) {
                                $pid = $getPurchaseData['pid'];
                                $cname = getCourseName($getPurchaseData['cid'], $con);
                                $amt = $getPurchaseData['cost'];
                                $dateofpurchase = $getPurchaseData['dateofpurchase'];
                                ?>
                                <tr>
                                    <td><?php echo getPid($pid); ?></td>
