<?php
/**
 * Written by Aan (aancodes@gmail.com) at Global BizConnect on 19/5/19 1:12 PM.
 */
...
//For every course taught by trainer, add up the count of students per course
$totalEnrollsQuery = "SELECT count(t1.uid) AS totalenrolls FROM cstudents as t1 INNER JOIN ctrainers as t2 ON (t1.cid = t2.cid AND t2.uid = $uid)";
$totalEnrollsResult = mysqli_query($con, $totalEnrollsQuery);
$totalEnrollsData = mysqli_fetch_array($totalEnrollsResult);
$totalEnrolls = $totalEnrollsData['totalenrolls'];

$previousMonthEnrollsQuery = "SELECT count(t1.uid) as previousmonthenrolls FROM cstudents as t1 INNER JOIN ctrainers as t2 ON (t1.cid = t2.cid AND t2.uid = $uid) WHERE t1.dateofjoining >= DATE_SUB(curdate(), INTERVAL 1 MONTH)";
$previousMonthEnrollsResult = mysqli_query($con, $previousMonthEnrollsQuery);
$previousMonthEnrollsData = mysqli_fetch_array($previousMonthEnrollsResult);
$previousMonthEnrolls = $previousMonthEnrollsData['previousmonthenrolls'];

$checkTrainingRatingsExistResult = mysqli_query($con, "SELECT COUNT(trid) AS totalreviews FROM treviews WHERE tuid = " . $uid);
$checkTrainingRatingsExistData = mysqli_fetch_array($checkTrainingRatingsExistResult);
if ($checkTrainingRatingsExistData['totalreviews'] > 0) {
    $trainerRatingQuery = "SELECT (SUM(rating)/COUNT(trid)) AS trainerrating FROM treviews WHERE tuid = $uid";
    $trainerRatingResult = mysqli_query($con, $trainerRatingQuery);
    $trainerRatingData = mysqli_fetch_array($trainerRatingResult);
    $averageTrainerRating = $trainerRatingData['trainerrating'];
} else {
    $averageTrainerRating = 0;
}

$checkCourseRatingsExistResult = mysqli_query($con, "SELECT COUNT(crid) AS totalreviews FROM creviews INNER JOIN ctrainers ON (creviews.cid= ctrainers.cid AND ctrainers.uid=" . $uid . ")");
$checkCourseRatingsExistData = mysqli_fetch_array($checkCourseRatingsExistResult);
if ($checkCourseRatingsExistData['totalreviews'] > 0) {
    $courseRatingQuery = "SELECT (SUM(rating)/COUNT(crid)) AS avgrating FROM creviews INNER JOIN ctrainers ON creviews.cid = ctrainers.cid AND ctrainers.uid = $uid";
    $courseRatingResult = mysqli_query($con, $courseRatingQuery);
    $courseRatingData = mysqli_fetch_array($courseRatingResult);
    $averageCourseRating = $courseRatingData['avgrating'];
} else {
    $averageCourseRating = 0;
}
$monthlyIncomeQuery = "SELECT SUM(cost) as income, MONTH(dateofpurchase) as `month` FROM cpurchases INNER JOIN ctrainers ON (cpurchases.cid = ctrainers.cid AND ctrainers.uid=$uid) GROUP BY MONTH(dateofpurchase) ORDER BY MONTH(dateofpurchase)";
$monthlyIncomeResult = mysqli_query($con, $monthlyIncomeQuery);
$monthlyIncome = array();
while ($monthlyIncomeData = mysqli_fetch_array($monthlyIncomeResult)) {
    $monthlyIncome[$monthlyIncomeData['month']] = $monthlyIncomeData['income'];
}
$monthlyIncomeString = "";
for ($month = 1; $month <= 12; $month++) {
    if (!isset($monthlyIncome[$month])) {
        ...
        $monthlyIncomeString = $monthlyIncomeString . $monthlyIncome[$month];
    }
    if ($month != 12) {
        $monthlyIncomeString = $monthlyIncomeString . ",";
    }
}
...
<div id="wrapper">
    ...
    <!-- Sidebar -->
    <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar" style="background: #662D91;">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">Your Performance</h1>
                            ...
                            <div class="card-body">
                                <div class="row no-gutters align-items-center">
                                    <div class="col mr-2">
                                        ...
                                        <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo $previousMonthEnrolls; ?></div>
                                    </div>
                                    <div class="col-auto">
                            ...
                            <div class="card-body">
                                <div class="row no-gutters align-items-center">
                                    <div class="col mr-2">
                                        ...
                                        <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo $totalEnrolls; ?></div>
                                    </div>
                                    <div class="col-auto">
                            ...
                            <div class="card-body">
                                <div class="row no-gutters align-items-center">
                                    <div class="col mr-2">
                                        ...
                                        <div class="row no-gutters align-items-center">
                                            <div class="col-auto">
                                                <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800"><?php echo $averageTrainerRating * 20; ?>
                                                    %
                                                </div>
                                            </div>
                                            <div class="col">
                                                <div class="progress progress-sm mr-2">
                                                    <div class="progress-bar bg-info" role="progressbar"
                                                         style="width: <?php echo $averageTrainerRating * 20; ?>%"
                                                         aria-valuenow="<?php echo $averageTrainerRating * 20; ?>"
                                                         aria-valuemin="0"
                            ...
                            <div class="card-body">
                                <div class="row no-gutters align-items-center">
                                    <div class="col mr-2">
                                        ...
                                        <div class="row no-gutters align-items-center">
                                            <div class="col-auto">
                                                <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800"><?php echo $averageCourseRating * 20; ?>
                                                    %
                                                </div>
                                            </div>
                                            <div class="col">
                                                <div class="progress progress-sm mr-2">
                                                    <div class="progress-bar bg-warning" role="progressbar"
                                                         style="width: <?php echo $averageCourseRating * 20; ?>%"
                                                         aria-valuenow="<?php echo $averageCourseRating * 20; ?>"
                                                         aria-valuemin="0"
                                                         aria-valuemax="100"></div>
                                                </div>
                                            </div>
                ...
                <div class="row">
                    <div class="container">
                        <div class="card shadow mb-4">
                            <!-- Card Header - Dropdown -->
                            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                <h6 class="m-0 font-weight-bold text-primary">Earnings Statement - All Time</h6>
                            </div>
                            <!-- Card Body -->
                            <div class="card-body">
