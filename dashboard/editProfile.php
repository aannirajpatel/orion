...
if (isset($_POST['fname']) && $_POST['fname'] != "") {
    ...
    $updateFnameQuery = "UPDATE user SET fname = '$newFname' WHERE uid=$uid";
    $updateFnameResult = mysqli_query($con, $updateFnameQuery) or die(mysqli_error($con));
}

if (isset($_POST['lname']) && $_POST['lname'] != "") {
    $newLname = mysqli_real_escape_string($con, $_POST['lname']);
    $updateLnameQuery = "UPDATE user SET lname = '$newLname' WHERE uid=$uid";
    $updateLnameResult = mysqli_query($con, $updateLnameQuery) or die(mysqli_error($con));
}

if (isset($_POST['email']) && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
    $checkDupEmailQuery = "SELECT count(*) AS numemails FROM user WHERE email='" . $_POST['email'] . "'";
    $checkDupEmailResult = mysqli_query($con, $checkDupEmailQuery) or die(mysqli_error($con));
    $checkDupEmailData = mysqli_fetch_array($checkDupEmailResult);
    $checkDupEmail = $checkDupEmailData['numemails'];
    if ($checkDupEmail == 0) {
        $updateEmailQuery = "UPDATE user SET email=" . $_POST['email'] . "WHERE uid=$uid";
        $updateEmailResult = mysqli_query($con, $updateEmailQuery) or die(mysqli_error($con));
...
if (isset($_POST['bio']) && $_POST['bio'] != "") {
    $newBio = mysqli_real_escape_string($con, $_POST['bio']);
    $updateBioQuery = "UPDATE user SET bio='$newBio' WHERE uid=$uid";
    $updateBioResult = mysqli_query($con, $updateBioQuery) or die(mysqli_error($con));
}

if (isset($_FILES['profileimg'])) {
    if ($_FILES['profileimg']['error'] != 0 && $_FILES['profileimg']['error'] != 4) {
        $message = "Error uploading the file. Please try uploading profile image again.";
        header("location:displayMessage.php?message=$message");
    }

    if ($_FILES['profileimg']['error'] != 4) {
        $extensionSplitter = preg_split("/\./", basename($_FILES['profileimg']['name']), 2);
        if (count($extensionSplitter) == 2) {
            $extension = $extensionSplitter[1];
        } else {
            $extension = "png";
        }
        $faddr = "./res/userimages/";
        if (!is_dir($faddr)) {
            mkdir($faddr, 0755, true);
        }
        $faddr = $faddr . "$uid" . "." . $extension;
        $filename = $uid . "." . $extension;
        if (file_exists($faddr)) {
            unlink($faddr);
        }

        move_uploaded_file($_FILES['profileimg']['tmp_name'], $faddr) or die("Error uploading. Please try adding the file again.");

        $updateProfileImgQuery = "UPDATE user SET profileImageFilename = '$filename' WHERE uid=$uid";
        $updateProfileImgResult = mysqli_query($con, $updateProfileImgQuery) or die(mysqli_error($con));
...
$bioQuery = "SELECT * FROM user WHERE uid=$uid";
                ...
                <div class="container">
                    <form method="post" action="" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                <input name="profileimg" class="form-control-file" type="file">
                            </div>
                        </div>
                        <div class="form-group">
                            <input class="bg-gradient-primary form-control text-white" type="submit"
